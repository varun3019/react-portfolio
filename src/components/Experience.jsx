...
<div>
    {EXPERIENCES.map((experience,index)=>(
        <div key={index} className='mb-8 flex flex-wrap lg:justify-center ' >
            ...
            <p className='mb-2 text-sm mt-5'>{experience.year}</p>
            </motion.div>
            ...
            {experience.technologies.map((tech,index)=>(
                <span key={index} className='mr-2  mt-4 rounded bg-neutral-900 px-2 py-1 text-sm font-medium block mb-2 xl:inline lg:block  '>
                    {tech}
                </span>
            ))}
            </motion.div>
